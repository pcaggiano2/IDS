...
class TrainerAnomaly():
    def __init__(self, args, splitter, detector, comp_loss, dataset, num_classes):
        ...
        self.chpt_base_dir = os.path.join(
            args.save_folder, args.project_name,"fold" + str(self.tasker.data.fold_idx))
        self.chpt_dir = os.path.join(self.chpt_base_dir, 'checkpoints')
        self.train_val_result_dir = os.path.join(self.chpt_base_dir, 'train_val_results')

        iot23_test_result_dir = os.path.join(self.chpt_base_dir, 'IoT23')
        iot_id20_test_result_dir = os.path.join(self.chpt_base_dir, 'IoTID20')
        iot_traces_test_result_dir = os.path.join(self.chpt_base_dir, 'IoT_traces')
        bot_iot_test_result_dir = os.path.join(self.chpt_base_dir, 'Bot-IoT')

        os.makedirs(self.chpt_base_dir, exist_ok=True)
        os.makedirs(self.chpt_dir, exist_ok=True)
        os.makedirs(self.train_val_result_dir, exist_ok=True)
        os.makedirs(iot23_test_result_dir, exist_ok=True)
        os.makedirs(iot_id20_test_result_dir, exist_ok=True)
        os.makedirs(iot_traces_test_result_dir, exist_ok=True)
        os.makedirs(bot_iot_test_result_dir, exist_ok=True)

        self.iot23_test_result_file = open(os.path.join(
            iot23_test_result_dir, f'result_IoT23.csv'), "w", newline='')
        self.iot23_test_writer = csv.writer(self.iot23_test_result_file)

        self.iot_id20_test_result_file = open(os.path.join(
            iot_id20_test_result_dir, f'result_IoTID20.csv'), "w", newline='')
        self.iot_id20_test_writer = csv.writer(self.iot_id20_test_result_file)
    
        self.iot_traces_test_result_file = open(os.path.join(
            iot_traces_test_result_dir, f'result_IoT_traces.csv'), "w", newline='')
        self.iot_traces_test_writer = csv.writer(self.iot_traces_test_result_file)

        self.bot_iot_test_result_file = open(os.path.join(
            bot_iot_test_result_dir, f'result_Bot-IoT.csv'), "w", newline='')
        self.bot_iot_test_writer = csv.writer(self.bot_iot_test_result_file)

        for writer in [self.iot23_test_writer, self.iot_id20_test_writer, self.iot_traces_test_writer, self.bot_iot_test_writer]:
            writer.writerow(['Set', 'Accuracy', 'Precision',
    ...
    def train_anomaly(self):
        ...
        train_result_file = open(os.path.join(
            self.train_val_result_dir, 'train_result.csv'), "w", newline='')
        train_writer = csv.writer(train_result_file)
        val_result_file = open(os.path.join(
            self.train_val_result_dir, 'validation_result.csv'), "w", newline='')
        val_writer = csv.writer(val_result_file)

        train_writer.writerow(['epoch', 'loss', 'struct_loss', 'feat_loss'])
        ...
        for e in tqdm(range(self.args.num_epochs)):
            ...
            train_writer.writerow(
                [e, loss_epoch, attr_error_epoch, stru_error_epoch])
    ...
    def test_anomaly(self, compute_thr=True):
        ...
        if not self.tasker.data.sequence:
            ...
            # self.run_test(split_name='bot_iot_benign',
            #                threshold=threshold)
            # self.run_test(split_name='bot_iot_malicious',
            #                threshold=threshold)
            # self.run_test(split_name='bot_iot_mixed',
            #                threshold=threshold)
        else:
            self.run_test(split_name='validation',
                           threshold=threshold)
        ...
        self.iot23_test_result_file.close()
        self.iot_id20_test_result_file.close()
        self.iot_traces_test_result_file.close()
        self.bot_iot_test_result_file.close()
    ...
    def run_test(self, split_name, threshold):
        ...
        if split_name == 'validation':
            ...
            writer = self.iot_traces_test_writer
        elif split_name == 'iot_id20_benign':
            epoch_name = None
            split = self.splitter.test_iotid20_benign
            writer = self.iot_id20_test_writer
        elif split_name == 'iot_id20_mixed':
            epoch_name = None
            split = self.splitter.test_iotid20_mixed
            writer = self.iot_id20_test_writer
        elif split_name == 'bot_iot_benign':
            epoch_name = None
            split = self.splitter.test_botiot_benign
            writer = self.bot_iot_test_writer
        elif split_name == 'bot_iot_malicious':
            epoch_name = None
            split = self.splitter.test_botiot_malicious
            writer = self.bot_iot_test_writer
        elif split_name == 'bot_iot_mixed':
            epoch_name = None
            split = self.splitter.test_botiot_mixed
            writer = self.bot_iot_test_writer
        ...
        average_pred_time = elapsed_time / len(split)
        ...
        writer.writerow([split_name, accuracy, precision, recall, f_score, tp, tn, fp, fn, average_pred_time])
