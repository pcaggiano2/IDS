...
class VGRNN(nn.Module):
    ...
    def __init__(self, args, activation, device='cpu', conv_type='GCN', bias=True):
        ...
        if conv_type == 'GCN':
            self.phi_x = nn.Sequential(nn.Linear(self.input_dim, self.hidden_dim, bias=bias), nn.ReLU())
            self.phi_z = nn.Sequential(nn.Linear(self.output_dim, self.hidden_dim, bias=bias), nn.ReLU())

            self.enc = GCNConv(self.hidden_dim + self.hidden_dim, self.hidden_dim, bias=bias)
            self.enc_mean = GCNConv(self.hidden_dim, self.output_dim, act=lambda x: x, bias=bias)
            self.enc_std = GCNConv(self.hidden_dim, self.output_dim, act=F.softplus, bias=bias)

            self.prior = nn.Sequential(nn.Linear(self.hidden_dim, self.hidden_dim, bias=bias), nn.ReLU())
            self.prior_mean = nn.Sequential(nn.Linear(self.hidden_dim, self.output_dim, bias=bias))
            self.prior_std = nn.Sequential(nn.Linear(self.hidden_dim, self.output_dim, bias=bias), nn.Softplus())

            self.rnn = graph_gru_gcn(self.hidden_dim + self.hidden_dim, self.hidden_dim, self.rnn_layer_num, bias=bias)

        elif conv_type == 'SAGE':
            self.phi_x = nn.Sequential(nn.Linear(self.input_dim, self.hidden_dim, bias=bias), nn.ReLU())
            self.phi_z = nn.Sequential(nn.Linear(self.output_dim, self.hidden_dim, bias=bias), nn.ReLU())

            self.enc = SAGEConv(self.hidden_dim + self.hidden_dim, self.hidden_dim, bias=bias)
            self.enc_mean = SAGEConv(self.hidden_dim, self.output_dim, act=lambda x: x, bias=bias)
            self.enc_std = SAGEConv(self.hidden_dim, self.output_dim, act=F.softplus, bias=bias)

            self.prior = nn.Sequential(nn.Linear(self.hidden_dim, self.hidden_dim, bias=bias), nn.ReLU())
            self.prior_mean = nn.Sequential(nn.Linear(self.hidden_dim, self.output_dim, bias=bias))
            self.prior_std = nn.Sequential(nn.Linear(self.hidden_dim, self.output_dim, bias=bias), nn.Softplus())

            self.rnn = graph_gru_sage(self.hidden_dim + self.hidden_dim, self.hidden_dim, self.rnn_layer_num, bias=bias)

        else:  # 'GIN':
            self.phi_x = nn.Sequential(nn.Linear(self.input_dim, self.hidden_dim, bias=bias), nn.ReLU())
            self.phi_z = nn.Sequential(nn.Linear(self.output_dim, self.hidden_dim, bias=bias), nn.ReLU())

            self.enc = GINConv(nn.Sequential(nn.Linear(self.hidden_dim + self.hidden_dim, self.hidden_dim, bias=bias), nn.ReLU()))
            self.enc_mean = GINConv(nn.Sequential(nn.Linear(self.hidden_dim, self.output_dim, bias=bias)))
            self.enc_std = GINConv(nn.Sequential(nn.Linear(self.hidden_dim, self.output_dim, bias=bias), nn.Softplus()))
