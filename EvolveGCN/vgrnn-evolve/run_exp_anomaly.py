...
if __name__ == '__main__':
    ...
    classifier = build_classifier(args, tasker).to(device=args.device)
        ...
        if args.off_line_test:
            if args.test_epoch != -1:
                ...
                gcnn = torch.load(os.path.join(
                    args.save_folder, args.project_name,'checkpoints', f"detector_{args.test_epoch}.pt"))

            else:
                print("Loading best detector ....")
                path = get_best_model(args=args,
                                      directory_path=os.path.join(args.save_folder, args.project_name, 'checkpoints'))

                gcnn = torch.load(path, map_location=torch.device('cpu'))
