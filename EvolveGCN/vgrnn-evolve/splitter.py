    ...
    def __init__(self, args, tasker):
        ...
        if tasker.is_static:  # For static datsets
            ...
            if isinstance(tasker, nct.Node_Cls_Tasker):
                ...
                if tasker.data.fold_idx != 0:
                    train = AnomalyDataset(
                        tasker=tasker,
                        path=tasker.data.cross_val_path,
                        split="train" + str(tasker.data.fold_idx))
                    print(f"Train len {len(train)}")
                    dev = AnomalyDataset(tasker=tasker,
                                        path=tasker.data.cross_val_path,
                                        split="val"+str(tasker.data.fold_idx))
                ...
                if not tasker.data.sequence:
                    test_botiot_benign = AnomalyDataset(tasker=tasker,
                                                        path=tasker.data.bot_iot_path,
                                                        split='Bot_IoT_benign')
                    print(f"test_botiot_benign len {len(test_botiot_benign)}")
                    test_botiot_malicious = AnomalyDataset(tasker=tasker,
                                                        path=tasker.data.bot_iot_path,
                                                        split='Bot_IoT_malicious')
                    print(f"test_botiot_malicious len {len(test_botiot_malicious)}")
                    test_botiot_mixed = AnomalyDataset(tasker=tasker,
                                                        path=tasker.data.bot_iot_path,
                                                        split='Bot_IoT_mixed')
                    print(f"test_botiot_mixed len {len(test_botiot_mixed)}")
                else:
                    test_botiot = AnomalyDataset(tasker=tasker,
                                                path=tasker.data.bot_iot_path,
                                                split="Bot_IoT")
                    print(f"test_botiot len {len(test_botiot)}")
                ...
                if not tasker.data.sequence:
                    test_botiot_benign = DataLoader(
                        test_botiot_benign, shuffle=False, num_workers=args.data_loading_params['num_workers'], batch_size=1, collate_fn=collate_func)
                    
                    test_botiot_malicious = DataLoader(
                        test_botiot_malicious, shuffle=False, num_workers=args.data_loading_params['num_workers'], batch_size=1, collate_fn=collate_func)
                    
                    test_botiot_mixed = DataLoader(
                        test_botiot_mixed, shuffle=False, num_workers=args.data_loading_params['num_workers'], batch_size=1, collate_fn=collate_func)
                else:
                    test_botiot = DataLoader(
                        test_botiot, shuffle=False, num_workers=args.data_loading_params['num_workers'], batch_size=1, collate_fn=collate_func)
            ...
            if not self.tasker.data.sequence:
                self.test_botiot_benign = test_botiot_benign
                self.test_botiot_malicious = test_botiot_malicious
                self.test_botiot_mixed = test_botiot_mixed
            else:
                self.test_botiot = test_botiot
...
class AnomalyDataset(Dataset):
    ...
    def __init__(self, tasker: nat.Anomaly_Detection_Tasker, path: str = None, split: str = 'train', representation: str = 'tdg_graph'):
        ...
        if "train" in split:
            self.data_dict_path = os.path.join(path, split + ".json")
        elif "val" in split:
            self.data_dict_path = os.path.join(path, split + ".json")
        ...
        elif split == "Bot_IoT_benign":
            self.data_dict_path = os.path.join(path, "test_benign.json")
        elif split == "Bot_IoT_malicious":
            self.data_dict_path = os.path.join(path, "test_malicious.json")
        elif split == "Bot_IoT_mixed":
            self.data_dict_path = os.path.join(path, "test_mixed.json")
        elif split == "Bot_IoT":
            self.data_dict_path = [os.path.join(path, "test_benign.json"),
        ...
        if not self.tasker.data.sequence or not isinstance(self.data_dict_path, list):
            ...
            for capture in self.data_dict.keys():
                ...
                graph_cnt = 0
                for sequence_indx, graph in enumerate(self.data_dict[capture]):
    ...
    def check_graph(self, data_dict_path, graph_name, capture_name):
        if self.mode == "train" or self.mode == "val" or self.mode == "test_mixed" or self.mode == "test_malicious" or self.mode == "test_benign" or self.mode =="test_iot23":
            graph_base_folder = self.tasker.data.graph_base_iot23_folder
        elif self.mode == "IoT_traces":
            graph_base_folder = self.tasker.data.graph_base_iot_traces_folder
        elif self.mode == "IoTID20_benign" or self.mode == "IoTID20_mixed" or self.mode == "IoTID20":
            graph_base_folder = self.data.graph_base_iot_id20_folder
        else:
            graph_base_folder = self.data.graph_base_bot_iot_folder
    ...
    def __getitem__(self, idx):
        ...
        if "train" in self.mode :
            ...
            graph_type = "mixed"
        elif self.mode == "Bot_IoT_benign":
            graph_type = "full_benign"
        elif self.mode == "Bot_IoT_malicious":
            graph_type = "full_malicious"
        elif self.mode == "Bot_IoT_mixed":
            graph_type = "mixed"
        else:
            graph_type = None
