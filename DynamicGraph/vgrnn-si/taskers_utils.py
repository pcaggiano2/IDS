...
def normalize_adj(adj, num_nodes):
    ...
    sp_tensor = torch.sparse.FloatTensor(idx.t(), vals.type(
        torch.float), torch.Size([num_nodes, num_nodes]))
...
def get_non_existing_edges(adj, number, tot_nodes, smart_sampling, existing_nodes=None):
    ...
    if smart_sampling:
        ...
        def sample_edges(num_edges):
            ...
            to_id = np.random.choice(
                existing_nodes, size=num_edges, replace=True)
