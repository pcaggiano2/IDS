...
class Node_Cls_Tasker():
    ...
    def build_get_node_feats(self, args, dataset):
        if args.use_2_hot_node_feats:
            max_deg_out, max_deg_in = tu.get_max_degs(
                args, dataset, all_window=True)
            ...
            def get_node_feats(i, adj):
                # [i] I'm ignoring the index since the features for Elliptic are static
    ...
    def get_sample(self, idx, test):
        ...
        for i in range(idx - self.args.num_hist_steps, idx+1):
            ...
            cur_adj = tu.normalize_adj(
                adj=cur_adj, num_nodes=self.data.num_nodes)
...
class Static_Node_Cls_Tasker(Node_Cls_Tasker):
    def __init__(self, args, dataset):
        ...
        self.adj_matrix = tu.get_static_sp_adj(
            edges=self.data.edges, weighted=False)

        if args.use_2_hot_node_feats:
            max_deg_out, max_deg_in = tu.get_max_degs_static(
                self.data.num_nodes, self.adj_matrix)
            self.feats_per_node = max_deg_out + max_deg_in
            ...
            self.nodes_feats = u.sparse_prepare_tensor(self.nodes_feats, torch_size=[
                                                       self.data.num_nodes, self.feats_per_node], ignore_batch_dim=False)

        else:
            self.feats_per_node = dataset.feats_per_node
            self.nodes_feats = self.data.node_feats

        self.adj_matrix = tu.normalize_adj(
            adj=self.adj_matrix, num_nodes=self.data.num_nodes)
        self.is_static = True
